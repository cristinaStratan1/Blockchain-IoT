...
//	The specific initial value of Ks, that is added manually is denoted by Kiks.
	...
	static ArrayList<Integer> IoTDevicesIDs;
...
//	 Random set of sequence numbers of session i frames to be used in session (i + 1)’s session key update
	...
	public void listenIoTDevices() throws MqttException {
		...
		IoTDevicesIDs = new ArrayList<Integer>();
	}
...
//			startAuth();
			...
			try {
				Multihash filePointer = Multihash.fromBase58(digestMessage[2]);
				byte[] fileContents = ipfs.cat(filePointer);
				byte[] two = "UPDATE ".getBytes();
				byte[] combined = new byte[fileContents.length + two.length];
				System.arraycopy(fileContents, 0, combined, 0, fileContents.length);
				System.arraycopy(two, 0, combined, fileContents.length, two.length);
				MqttMessage updateContent = new MqttMessage(combined);
				thisClient.publish("EdgeDevice", updateContent);
			} catch (Exception e) {
				System.out.println("CID not correct");
			}

		} else if (digestMessage[0].equals("auth_request")) {
			String decodedMAC = ccm.encode(Kiks,
					digestMessage[0] + " " + digestMessage[1] + " " + digestMessage[2] + " " + digestMessage[3]);
			if (digestMessage[digestMessage.length - 1].equals(decodedMAC)) {
				IoTDevicesIDs.add(Integer.parseInt(digestMessage[1]));
...
//			startAuth();
	...
	static String[] generateSetOfSeqNumbers(int randoms) {
		ccm = new CCMImpl();
		String arrayInt[] = { "234365208", "247058986", "859950640", "886209246", "1758019480", "1959642116",
				"1161690206", "1406042517", "734704895", "640922405" };
		return arrayInt;
	}
